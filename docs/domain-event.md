...
Để chuẩn hóa kiến trúc cho hệ thống quản lý hạ tầng quy mô lớn, tên miền tổng quát **Server Management System (SMS-SMS)** được bóc tách và phân rã thành các miền con (Subdomains) và các ngữ cảnh bị giới hạn (Bounded Contexts) biệt lập. Mô hình áp dụng nguyên tắc cô lập dữ liệu (Data Isolation) nhằm đảm bảo khả năng mở rộng quy mô lớn (Horizontal Scaling) mà không gây ra xung đột hay thắt nút cổ chai tại cơ sở dữ liệu.
...
Việc thống nhất ngôn ngữ thống nhất là bắt buộc để triệt tiêu sự mơ hồ giữa đội ngũ phát triển sản phẩm (Product) và kỹ sư kiến trúc (Engineers). Các thuật ngữ trong hệ thống SMS-SMS được định nghĩa tường minh dưới đây:
...
Dưới đây là bảng đặc tả chi tiết toàn bộ các luồng tiến trình nghiệp vụ cốt lõi của hệ thống SMS-SMS
...
* **Mục đích phục vụ nghiệp vụ:** Tính toán chính xác tỷ lệ sẵn sàng hoạt động (%) của một thiết bị độc lập hoặc toàn bộ cụm hạ tầng công ty SMS mà không bị tính phạt oan khoảng thời gian sau khi server đã bị xóa cứng ra khỏi hệ thống.
