...
Trong hạ tầng công nghệ thông tin quy mô lớn của Công ty Cổ phần An ninh mạng Viettel (Viettel Cyber Security \- VCS), việc giám sát trạng thái hoạt động của hạ tầng thiết bị là nhiệm vụ sống còn. Hệ thống **Server Management System (VCS-SMS)** được định hướng phát triển nhằm mục đích quản lý tập trung và kiểm tra tự động trạng thái hoạt động (On/Off) của toàn bộ danh sách server thuộc công ty.
...
Hồ sơ các quyết định kiến trúc lớn dưới đây đóng vai trò định hình khung kỹ thuật cho dự án VCS-SMS:

## **ADR-001: LỰA CHỌN PHƯƠNG THỨC ĐỊNH NGHĨA TRẠNG THÁI SERVER (HEALTH CHECK TYPE)**

* **Mã quyết định:** ADR-001  
* **Trạng thái (Status):** Đã phê duyệt (Approved)  
...
* **Hệ quả (Consequences)**:  
  ...
  * *Biện pháp giảm thiểu:* Hướng dẫn đội vận hành hạ tầng whitelist cấu hình cho phép IP của hệ thống VCS-SMS đi qua tường lửa của các server thành viên.
...
* **Trạng thái (Status):** Chờ phê duyệt (Waiting for approval)  
