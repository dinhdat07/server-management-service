...
redis = container "Cache & Lock" "Stores sessions and distributed locks." "Redis 7" "Database"
elastic = container "Log Storage" "Stores time-series ping logs and metrics." "Elasticsearch 8" "Database"
...
backend = container "Backend Application (API)" "Provides REST/gRPC endpoints and handles core business logic." "Go" {
    ...
    reportingComp = component "Reporting Component" "Generates PDF/Excel reports and serves them." "Go Module"
    ...
    group "Server Management Module (Code Level)" {
        ...
        serverService = component "Server Service" "Handles Business Logic & Excel Parsing" "Go"
...
monitorWorker = container "Monitoring Worker" "Background process that pings servers continuously." "Go Worker" {
    ...
    group "Monitoring Module (Code Level)" {
        ...
        monitoringService = component "Monitoring Service" "Evaluates ping results" "Go"
...
scheduler = container "Daily Scheduler" "Background process that triggers periodic jobs." "Go Cron" {
    // High-Level
    schedulerComp = component "Scheduler Component" "Triggers daily reports and other chron jobs." "Go Module"
